...
export default function StudentProfilePage() {
    ...
    async function loadProfile() {
        ...
        const { data, error } = await supabase
            .from('users')
            .select('id, full_name, email, roll_no, department_id, semester, scheme, phone, is_active, created_at, departments(name)')
            .eq('id', user.id)
            .single()
    ...
    async function handleSave() {
        ...
        const { error } = await (supabase.from('users') as any).update({
            ...
            role_id: 4, // Ensure student role
        }).eq('id', profile.id)

        if (error) {
            if (error.code === '23505') {
                const msg = error.message || ''
                if (msg.includes('idx_users_dept_sem_roll')) {
                    toast.error('Roll number already taken in your department/semester')
                } else {
                    toast.error('A unique constraint was violated. Please check your data.')
                }
            } else {
    ...
    const infoFields = [
        ...
        { label: 'Department', value: (profile as any).departments?.name ?? 'N/A', icon: '🏛️', missing: !profile.department_id },
    ...
    const hasMissing = !profile.roll_no || !profile.department_id || !profile.semester
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-end', gap: '1rem' }}>
                        ...
                        <div style={{ paddingBottom: '0.25rem' }}>
                            ...
                            <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem', margin: 0 }}>
                                {profile.roll_no ? `Roll No: ${profile.roll_no}` : 'No Roll No'} · {(profile as any).departments?.name ?? 'N/A'}
                            </p>
                        </div>
                    </div>
                ...
                <div className="modal-overlay" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
                    <div className="glass-card" style={{ width: '100%', maxWidth: 400, padding: '2rem' }}>
                        ...
                        <div style={{ marginTop: '1.5rem', display: 'flex', gap: '0.75rem' }}>
                            <button onClick={handleChangePassword} disabled={saving} className="btn-primary" style={{ flex: 1 }}>Update Password</button>
