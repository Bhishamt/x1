...
type CorrectionRow = {
    ...
    results: { subjects: { subject_code: string; subject_name: string } | null } | null
...
export default function AdminCorrectionsPage() {
    ...
    async function loadUserContext() {
        ...
        if (user) {
            ...
            if (data) {
                ...
                if (profile.role_id >= 3 && profile.department_id) setDepartmentSearch(profile.department_id)
            }
        }
    }
        ...
        const { data } = await supabase
            ...
            .select(`
                *,
                ...
                results(subjects(subject_code, subject_name))
            `)
    ...
    async function searchStudentResults() {
        ...
        const { data: student } = await supabase
            ...
            .select('id')
            .eq('roll_no', rollNum)
            .eq('department_id', departmentSearch)
            .eq('role_id', 4)
            .single()
        ...
        const { data, error } = await supabase
            .from('results')
            .select('id, student_id, subject_id, exam_type, marks_obtained, max_marks, semester, academic_year, users!results_student_id_fkey!inner(full_name, roll_no, department_id, semester), subjects(subject_code, subject_name)')
            .eq('student_id', sid)
                    ...
                    <tbody>
                        {corrections.map(c => (
                            <tr key={c.id}>
                                ...
                                <td>
                                    <div style={{ fontWeight: 600 }}>{c.results?.subjects?.subject_code ?? '—'}</div>
                                    <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{c.results?.subjects?.subject_name}</div>
                                </td>
                                <td>
                                ...
                                <div style={{ maxHeight: 200, overflowY: 'auto', border: '1px solid var(--border)', borderRadius: '0.5rem' }}>
                                    {studentResults.map(r => (
                                        <div key={r.id} onClick={() => { setSelectedResult(r); setNewMarks(String(r.marks_obtained)) }}
                                            ...
                                            <span>{r.subjects?.subject_code ?? '—'}</span>
                        ...
                        {selectedResult && (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                <div style={{ padding: '0.75rem', borderRadius: '0.5rem', background: 'rgba(59,130,246,0.1)', fontSize: '0.8rem', border: '1px solid rgba(59,130,246,0.3)' }}>
                                    <strong>{selectedResult.users?.full_name}</strong> · {selectedResult.subjects?.subject_code} · Current: {selectedResult.marks_obtained}/{selectedResult.max_marks}
