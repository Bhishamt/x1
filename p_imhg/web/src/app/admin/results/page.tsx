...
type ResultRow = {
    ...
    users: { full_name: string; roll_no: number | null; department: string | null; semester: number | null } | null
}
...
export default function AdminResultsPage() {
    ...
    useEffect(() => { loadResults() }, [filterDept, filterSem, filterRollNo, userDept])
    ...
    async function loadUserContext() {
        ...
        if (user) {
            ...
            if (data) {
                ...
                if (profile.role_id >= 3 && profile.department) setDepartment(profile.department)
            }
        }
    }
    ...
    const loadResults = useCallback(async () => {
        let q = supabase
            .from('results')
            .select('*, users!results_student_id_fkey!inner(full_name,roll_no,department,semester)')

        let targetDept = filterDept
        if (userRoleId >= 3 && userDept) targetDept = userDept

        if (targetDept) q = q.eq('users.department', targetDept)
    ...
    }, [filterDept, filterSem, filterRollNo, userRoleId, userDept, supabase])

    const loadSubjects = useCallback(async () => {
        if (!department || !semester || !scheme) return
        ...
        if (semester === 1 || semester === 2) {
            query = query.in('department', [department, 'COMMON'])
        } else {
    ...
    }, [department, semester, scheme, supabase])

    useEffect(() => {
        if (department && semester && scheme) loadSubjects()
    }, [department, semester, scheme, loadSubjects])

    async function lookupStudent() {
        if (!rollNo.trim()) { toast.error('Enter a roll number'); return }
        const q = supabase.from('users').select('id, full_name, roll_no, academic_year, scheme').eq('role_id', 5)
        ...
        subjects.forEach(s => { emptyMarks[s.subject_code] = '' })
        setMarks(emptyMarks)
    }
    ...
    function validateMarks(): boolean {
        ...
        for (const sCode of Object.keys(marks)) {
            const val = marks[sCode]
    ...
    async function submitResults() {
        ...
        try {
            const { data: { user } } = await supabase.auth.getUser()
            const inserts = filledMarks.map(([sCode, val]) => ({
                ...
                <>
                    <div className="glass-card" style={{ padding: '1.5rem', marginBottom: '1.5rem', display: 'flex', gap: '1rem', alignItems: 'flex-end', flexWrap: 'wrap' }}>
                        <div>
                            ...
                            <select className="input-dark" value={userRoleId >= 3 && userDept ? userDept : filterDept} disabled={userRoleId >= 3 && !!userDept} onChange={e => setFilterDept(e.target.value)}>
                        ...
                        <tbody>
                            {results.map(r => (
                                <tr key={r.id}>
                                    ...
                                    <td>
                                        <div style={{ fontSize: '0.85rem', fontWeight: 600 }}>{r.subject_code}</div>
                                    </td>
                ...
                <div className="glass-card" style={{ padding: '2rem' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
                        ...
                        <div>
                            ...
                            <select className="input-dark" value={department} disabled={userRoleId >= 3 && !!userDept}
                                onChange={e => { setDepartment(e.target.value); setStep(Math.max(step, 3)) }}>
                                ...
                                <>
                                    <div style={{ overflowX: 'auto', marginBottom: '1.5rem' }}>
                                        <table className="table-dark">
                                            ...
                                            <tbody>
                                                {subjects.map(s => {
                                                    const val = marks[s.subject_code] ?? ''
                                                    const num = Number(val)
                                                        ...
                                                        <tr key={s.id}>
                                                            ...
                                                            <td>
                                                                <input className="input-dark" type="number" min={0} max={maxMarks} step="0.5"
                                                                    ...
                                                                    onChange={e => setMarks(m => ({ ...m, [s.subject_code]: e.target.value }))}
